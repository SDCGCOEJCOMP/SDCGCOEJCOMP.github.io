...
function Particle() {
	...
	this.update = function (ms) {
		...
		this.x += this.velocityX * ms / 500.0;
		this.y += this.velocityY * ms / 500.0;
...
function createExplosion(x, y, color) {
	...
	for (var angle = 0; angle < 360; angle += Math.round(360 / count)) {
		...
		// size of particle
		particle.radius = randomFloat(1, 7);

		particle.color = color;

		// life time, the higher the value the faster particle will die
		particle.scaleSpeed = randomFloat(0.1, 0.3);
...
window.addEventListener("load", function () {
	...
	button.addEventListener("click", function () {
        ...
        createExplosion(x, y, "#f73140");
        createExplosion(x, y, "#30a89c");
		createExplosion(x, y, "#00dfff");
        createExplosion(x, y, "#dda0dd");
